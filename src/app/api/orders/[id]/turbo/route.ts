...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await sendOrderToTurbo({
    government,
    area,
    address: body.address ?? order.customer.address ?? "",
    receiver: body.receiver ?? body.customerName ?? order.customer.name,
    phone1: body.phone1 ?? body.phone ?? order.customer.phone ?? "",
    phone2: body.phone2 ?? body.whatsappNumber ?? order.customer.whatsappNumber ?? undefined,
